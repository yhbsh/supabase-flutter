...
class StorageBucketApi {
  ...
  Future<Bucket> getBucket(String id) async {
    final FetchOptions options = FetchOptions(headers: headers);
    final response = await storageFetch.get('$url/bucket/$id', options: options);
  ...
  Future<String> createBucket(
    ...
    final response = await storageFetch.post(
      '$url/bucket',
      {
        ...
        if (bucketOptions.fileSizeLimit != null) 'file_size_limit': bucketOptions.fileSizeLimit,
        if (bucketOptions.allowedMimeTypes != null) 'allowed_mime_types': bucketOptions.allowedMimeTypes,
      },
      options: options,
    );
  ...
  Future<String> emptyBucket(String id) async {
    final FetchOptions options = FetchOptions(headers: headers);
    final response = await storageFetch.post('$url/bucket/$id/empty', {}, options: options);
